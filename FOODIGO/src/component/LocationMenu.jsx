...
import { Loading, locationDataApi, locationNameApi, swiggyBaseRestaurantOriginApi } from "../utils/Constant";
import { useNavigate } from "react-router-dom";
import { useDispatch } from "react-redux";
import { updateLat, updateLng, updateLocationData, updateResData } from "../utils/restaurantDataSlice";
...
function LocationMenu({ setShowLoactionMenu }) {
  ...
  async function fetchRestaurantDataByLocation(lat, lng) {
    const response = await fetch(swiggyBaseRestaurantOriginApi + lat + '&lng=' + lng + '&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING');
    const json = await response.json();
    if(json?.data?.cards[0]?.card?.card?.id == 'swiggy_not_present'){
      dispatch(updateResData('swiggy_not_present'))
    } else {
      dispatch(updateResData(json?.data?.cards));
      dispatch(updateLat(lat));
      dispatch(updateLng(lng));
    }
  }

  async function fetchLocationData(placeId) {
    const response = await fetch(locationDataApi + placeId);
    const json = await response.json();    
    dispatch(updateLocationData(json?.data[0]?.formatted_address));

    fetchRestaurantDataByLocation(json?.data[0]?.geometry?.location?.lat, json?.data[0]?.geometry?.location?.lng)
  }
  
  function loadAreaData(placeId){
    hideLoactionMenu();
    navigate("/")
    dispatch(updateResData(null));
    fetchLocationData(placeId);
  }

  async function fetchDataByKey() {
    setLoad(true);
    const response = await fetch(locationNameApi + searchKey);
    const json = await response.json();
    setLoad(false);
