...
import { restaurantNameApi, SearchedRestaurantApi, searchSuggetionImageDatabase } from "../utils/Constant";
...
function SearchedSuggestResList() {
  ...
  async function loadLiveSuggetion() {
    setLoad(true);
    const response = await fetch(restaurantNameApi + lat + "&lng=" + lng + "&str=" + searchKey + "&trackingId=undefined&includeIMItem=true" );
    const json = await response.json();
    const filterRestaurentData = json?.data?.suggestions?.filter((item) => item?.type == "RESTAURANT");
    if (filterRestaurentData != 0) {
      setSuggetionData(filterRestaurentData);
    } else {
      setSuggetionData(null);
      setNotFound(true);
    }
