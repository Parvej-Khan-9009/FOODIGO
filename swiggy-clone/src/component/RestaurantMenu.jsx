...
import useFetchDataDynamically from "../utils/useFetchDataDynamically";

function RestaurentMenu() {
  const [showIndex, setShowIndex] = useState(0);
  const { menuId } = useParams();
  let menuData = useFetchDataDynamically(menuDataBase, menuId);
  const restartRestaurantList = useRef(null);
  const differentRestaurantInfo = useSelector(
    (store) => store.cart.differentRestaurentData
  );
